...
1. 每次对话开始时**必须先调用** `pipeline-orchestrator.advance_pipeline()` 获取当前阶段指令
2. **不要自行判断**当前应该执行哪个 skill — 以 MCP tool 返回值为准
3. 每个 stage 完成后**必须调用** `pipeline-orchestrator.complete_stage()` 验证并推进
4. 如果返回值中 `parallel=true`，按 `subagent_tasks` dispatch subagents（参考 `dispatching-parallel-agents` skill）
5. 如果返回值中 `requires_model_switch=true`，遵循 `sdp-protocol` 的 handoff 模板
6. 硬卡点（`is_hard_checkpoint=true`）无论 autopilot 状态如何，都必须暂停等待用户确认
7. 如果返回值中 `rollback.enabled=true`，在条件满足时调用 `complete_stage(result="rollback")` 触发回退
8. 读取 `user_level` 字段，按 Expert/Intermediate/Novice 调整行为
...
决策点行为差异：
- **Expert**: 展示信息 + 选项，用户自行判断
- **Intermediate**: 展示信息 + AI 推荐 + 理由，用户可覆盖
- **Novice**: AI 给出有倾向性的推荐，附 "为什么推荐这个" + "如果不选这个的风险"
