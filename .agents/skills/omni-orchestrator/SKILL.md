...
3. 继续执行当前 pipeline，所有卡点自动 approve
...
在每个人机卡点（Scope Freeze / Corpus Freeze / Idea Approval / Review Arena）：

1. 读取 `project_state.json` 的 `"autopilot"` 字段
2. **autopilot=true**：输出 1-2 行 summary → 自动继续下一步
3. **autopilot=false**（默认）：展示详情 → 等待用户确认
